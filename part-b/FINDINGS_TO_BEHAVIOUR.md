# From finding to behavior — what the prototype actually does, and what it refuses to do
...
Today's search behaves like a slot machine that never looks at the catalog. It draws a result count
...
2. **Look at what the city actually holds** — retrieve from `deals.csv` for the selected city. The count is a fact about catalog, never a number the system invents.
3. **Say out loud what it just did** — translated, corrected, matched by category, or nothing here.

Every behavior below is one of those three steps applied to one finding. Nothing else is going on.
...
|---|---|---|---|
| The city holds nothing of the kind (helicopter, balloon, skydiving, rafting, supercar, shark diving) | 1,512 | 57% | **No.** Needs catalog. |
...
0.58%, no-catalog 0.00% (1,512 searches, 0 clicks, 0 purchases — not one).

---

## Pillar 1 — Truthful retrieval

*The count is a fact about the catalog, not a number the system produces.*

| Finding | What happens today | Real example | What the prototype does | Fixable |
|---|---|---|---|---|
| **F2 — Retrieval ignores the catalog** | The number of results has no relationship to matching catalog (r = +0.02, robust under three independent definitions of "matching"). | — | Result count = the number of deals in that city whose concept matches. There is no other source for the number. | **Fully** |
| **F7 — Impossible results** | 585 searches (6.5%) returned more results than the city holds deals in total. | `gym` in Manchester returned up to **35**. Manchester's entire catalog is **19 deals**, of which 3 are fitness. | Hard cap asserted in code: `results ≤ deals in this city`. The prototype returns **3**. | **Fully** |
...
| **F5 — Vocabulary gap** | The catalog names deals in generic package language; customers search in specific product language. Title-string matching therefore misses. | `sushi` in **Madrid**: 44 searches, 3 purchases. Madrid holds **zero deals naming sushi**. The 7 nearest things are generic dining packages — `Cena con Maridaje`, `Menú de 3 Platos para Dos`. | Two-level answer. If the catalog names the concept, match it. If it only carries the category, return the category **and label it as such**: *"No deal in Madrid is named sushi. 7 dining deals, shown as dining."* The prototype never dresses a dining package up as a sushi result. | **Partly.** Search can be honest about it; only merchandising can retitle the deals |
...
- **Nothing in the catalog matches any of them.** Checked all 568 deals across all five markets
  and all 19 cities: there is no helicopter, balloon, skydiving, rafting, supercar or shark-diving
  deal anywhere in the dataset. The catalog is five categories deep — massage, dining,
...
- **Therefore "try the next city" does not exist as a fallback for this finding.** There is no
  ...
  ever have meant "same market" anyway.) The nearest-city fallback is real for a thin catalog —
...
- **Even the price band is wrong for it.** The most expensive deal in the entire catalog is
...
### What the prototype does, and how it is labeled
...
2. **One adjacent thing that genuinely exists, labeled as different — not as a substitute.**
   Mapped by hand, and the map is a judgment call, so the UI says so:
   ...
   Session** (the nearest adrenaline experience the catalog has at all); `shark diving` →
   **Guided City Tour**. Showing one deal labeled *"a different experience, not a replacement"*
   is more honest than three labeled "you might also like".
   ...
   *Revised 2026-08-31.* The last three were originally left empty, on the grounds that showing
   ...
   catalog holds, and the card says in as many words that **which substitute anyone would accept
   is not in the data** and that the mapping is a judgment while the demand figure beside it is
   measured.
3. **A way back into the catalog, and a way to register the demand.** Two things, in one block:
...
of nine unrelated deals: one labeled alternative reads as judgment, nine read as desperation. And
...
- **Prices labeled USD** across five non-US markets. Shown as given.

---

## Summary — the three-line version

1. **Two of every five searches that return nothing today are recoverable by search alone** (1,121 of 2,633). The prototype recovers them by understanding intent and reading real catalog.
...
3. **Everything else — the impossible counts, the instability, the language failures — is a retrieval layer that never checks the catalog.** Making it check is a small change with the largest measurable effect in the data.
